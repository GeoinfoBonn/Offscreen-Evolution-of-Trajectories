...
//	private static final String format = "%1$tY-%1$tm-%1$td %1$tH:%1$tM:%1$tS.%1$tL %4$s %2$s: %5$s%6$s%n";
	private static final String format = "%1$s %2$tb %2$td, %2$tY %2$tl:%2$tM:%2$tS %2$Tp %3$s %5$s: %6$s%7$s%n";
	...
	public String format(LogRecord record) {
		...
		case "INFO":
			return String.format(format, ANSI_BLACK, dat, source, record.getLoggerName(),
		...
		case "SEVERE":
			return String.format(format, ANSI_RED, dat, source, record.getLoggerName(),
					record.getLevel().getLocalizedName(), message + ANSI_RESET, throwable);
		default:
			return String.format(format, ANSI_BLUE, dat, source, record.getLoggerName(),
					record.getLevel().getLocalizedName(), message + ANSI_RESET, throwable);
		}
	}
}